...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Apollon.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Apollond.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeapollonamount=<n> " + _("Keep N Apollon anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Apollon is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Apollon is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Apollon") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Apollon to complete") << "\n";
    ...
    nAnonymizeApollonAmount = GetArg("-anonymizeapollonamount", 0);
    if(nAnonymizeApollonAmount > 999999) nAnonymizeApollonAmount = 999999;
    if(nAnonymizeApollonAmount < 2) nAnonymizeApollonAmount = 2;
