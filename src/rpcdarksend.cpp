...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Apollonaddress> <amount>\n"
            "Apollonaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <Apollonaddress> <amount>\n"
            "Apollonaddress, denominate, or auto (AutoDenominate)"
            "<amount> is type \"real\" and will be rounded to the nearest 0.1"
            + HelpRequiringPassphrase());

    CApolloncoinAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  stop         - Stop masternode configured in Apollon.conf\n"
                ...
                "  vote         - Vote on a Apollon initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CApolloncoinSecret(secret).ToString();
    }
    ...
    if(strCommand == "status")
    {
        ...
        if (pmn) mnObj.push_back(Pair("pubkey", CApolloncoinAddress(pmn->pubkey.GetID()).ToString()));
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Apollon initiative (can be additionally filtered, partial match)\n"
